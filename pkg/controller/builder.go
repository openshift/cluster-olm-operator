...
func updateEnv(con *corev1.Container, name, value string) ([]corev1.EnvVar, error) {
	for _, e := range con.Env {
		if e.Name == name {
			return con.Env, fmt.Errorf("unexpected environment variable %q=%q in container %q while building manifests", e.Name, e.Value, con.Name)
		}
	}
	if value == "" {
		return con.Env, nil
	}
	klog.FromContext(context.Background()).WithName("builder").V(1).Info("Updated environment", "container", con.Name, "key", name, "value", value)
	return append(con.Env, corev1.EnvVar{Name: name, Value: value}), nil
}

func setProxyEnvs(con *corev1.Container, vars map[string]string) error {
	var err error
	for name, value := range vars {
		con.Env, err = updateEnv(con, name, value)
		if err != nil {
			return err
		}
	}
...
func UpdateDeploymentProxyHook(pc clients.ProxyClientInterface) deploymentcontroller.DeploymentHookFunc {
	return func(_ *operatorv1.OperatorSpec, deployment *appsv1.Deployment) error {
		...
		for i := range deployment.Spec.Template.Spec.InitContainers {
			err = setProxyEnvs(&deployment.Spec.Template.Spec.InitContainers[i], vars)
			if err != nil {
				return err
			}
		}
		for i := range deployment.Spec.Template.Spec.Containers {
			err = setProxyEnvs(&deployment.Spec.Template.Spec.Containers[i], vars)
			if err != nil {
				return err
			}
		}
